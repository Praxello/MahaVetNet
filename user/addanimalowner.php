	 ...
	 $adharidtemp = $adharid;
		   ...
		   $tempaddress = mysqli_real_escape_string($conn,$address);
			...
			$rowsAffected=mysqli_affected_rows($conn);
				if($rowsAffected==1)
				{
					  $academicQuery = mysqli_query($conn,"select * from  animal_owner_master where branchid=$branchid order by ownerid desc");
								...
								while($academicResults = mysqli_fetch_assoc($academicQuery))
									{
										...
										if($academicResults['accCreationDate']== "$currentDate")
										{
											$farmerId = $academicResults['ownerId'];
										}
									}
							}
						}
					$response = array('Message'=>"Animal Owner added Successfully","Data"=>$records,"NewFarmerId"=>$farmerId,'Responsecode'=>200);	
				}
				else
				{

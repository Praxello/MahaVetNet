<?php
	 ...
	 {
			$query = mysqli_query($conn,"update vaccination_master set isdeleted = 1 where treatmentid=$treatmentid");
			...
			$rowsAffected=mysqli_affected_rows($conn);
				//if($rowsAffected==1)
				{
					...
					$checkquery = mysqli_query($conn,"delete from  fees_master where animalId=$animalid and visitDate='$visitdate' and typeOfPayment like '%Vaccination%'");
					  ...
					  // $academicQuery = mysqli_query($conn,"select * from  vaccination_master where isdeleted=0 and branchid = $branchid");

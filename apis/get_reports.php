...
if(isset($_POST['branchId']) && isset($_POST['reportType']) && isset($_POST['year']) && isset($_POST['month']) ){
    if($reportType == 1){
        ...
        $sql = "SELECT bm.centre_type,mm.visitDate AS Visit_Date ,am.animalName, aom.firstName AS FirstName, aom.lastName AS LastName,aom.address AS ownerAddress,aom.category AS Category ,am.animalName,am.specie AS Species,  am.breed AS Breed, mm.samples AS Samples,mm.treatment 
        FROM medication_master  mm 
            JOIN animal_master am ON mm.animalId = am.animalId 
            JOIN animal_owner_master aom ON am.ownerId = aom.ownerId 
            JOIN branch_master bm ON bm.branchId = mm.branchId
        ...
        $sql = "SELECT mm.visitDate AS Visit_Date ,am.animalName, aom.firstName AS FirstName, aom.lastName AS LastName,aom.address AS ownerAddress,aom.category AS Category ,am.specie AS Species,  am.breed AS Breed, mm.samples AS Samples,mm.treatment 
        FROM medication_master  mm 
            JOIN animal_master am ON mm.animalId = am.animalId 
            JOIN animal_owner_master aom ON am.ownerId = aom.ownerId 
            where mm.branchId = $branchId
        ...
        //cash register
        $sql = "SELECT fm.visitDate,am.animalName,fm.feesAmount,aom.firstName,aom.lastName,aom.mobile,aom.category,aom.address,fm.typeOfPayment
        FROM fees_master  fm 
        ...
        $sql = "SELECT ipd.visitDate,am.animalName,aom.firstName,aom.lastName,aom.address,aom.category,am.specie,am.breed,am.dateOfBirth,am.gender,am.weight,ipd.symptoms,ipd.diagnosis,ipd.treatment,ipd.dischargeDate
        ...
        $sql = "SELECT mm.visitDate,am.animalName , aom.firstName , aom.lastName,aom.address ,aom.category ,am.specie,  am.breed , mm.samples AS Samples,am.gender,am.dateOfBirth,am.weight,mm.symptoms,mm.diagnosis,mm.treatment 
        FROM medication_master  mm 
            JOIN animal_master am ON mm.animalId = am.animalId 
            JOIN animal_owner_master aom ON am.ownerId = aom.ownerId 
            where mm.branchId = $branchId
